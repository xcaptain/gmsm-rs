// 商密4 对称加密算法
// https://web.archive.org/web/20070710015158/http://www.oscca.gov.cn/UpFile/200621016423197990.pdf
// https://github.com/windard/sm4/blob/master/C/sm4.c
...
fn generate_sub_keys(key: [u8; 16]) -> [u32; 32] {
    let mut sub_keys = [0_u32; 32];
    let mut b = [0_u32; 4];
    b = permute_initial_block(b, key);
    b[0] ^= FK[0];
    b[1] ^= FK[1];
    b[2] ^= FK[2];
    b[3] ^= FK[3];
    for i in 0..32 {
        sub_keys[i] = feistel0(b[0], b[1], b[2], b[3], CK[i]);
        b[0] = b[1];
        b[1] = b[2];
        b[2] = b[3];
        b[3] = sub_keys[i];
...
fn permute_initial_block(b: [u32; 4], block: [u8; 16]) -> [u32; 4] {
    let mut arr = b;
    for i in 0..4 {
        arr[i] = (((block[i * 4] as u32) << 24) as u32)
            | (((block[i * 4 + 1] as u32) << 16) as u32)
            | (((block[i * 4 + 2] as u32) << 8) as u32)
            | (block[i * 4 + 3] as u32)
    }
    arr
}
...
impl Sm4Cipher {
    ...
    pub fn encrypt(&self, src: [u8; 16]) -> [u8; 16] {
        encrypt_block(self.sub_keys, self.block1, self.block2, src)
    }

    pub fn decrypt(&self, src: [u8; 16]) -> [u8; 16] {
        decrypt_block(self.sub_keys, self.block1, self.block2, src)
    }
}

fn get_ulong_be(start: u32, x: u32) -> u32 {
    start
        ^ SBOX0[(x & 0xff) as usize]
        ^ SBOX1[((x >> 8) & 0xff) as usize]
        ^ SBOX2[((x >> 16) & 0xff) as usize]
        ^ SBOX3[((x >> 24) & 0xff) as usize]
}

// 加密部分
fn encrypt_block(sub_keys: [u32; 32], b: [u32; 4], r: [u8; 16], src: [u8; 16]) -> [u8; 16] {
    let mut b = permute_initial_block(b, src);
    for i in 0..8 {
        let s = &sub_keys[4 * i..4 * i + 4];
        let mut x = b[1] ^ b[2] ^ b[3] ^ s[0];
        b[0] = get_ulong_be(b[0], x);
        x = b[0] ^ b[2] ^ b[3] ^ s[1];
        b[1] = get_ulong_be(b[1], x);
        x = b[0] ^ b[1] ^ b[3] ^ s[2];
        b[2] = get_ulong_be(b[2], x);
        x = b[1] ^ b[2] ^ b[0] ^ s[3];
        b[3] = get_ulong_be(b[3], x);
    }
    let nb = [b[3], b[2], b[1], b[0]];
    permute_final_block(r, nb)
}

// 解密部分
fn decrypt_block(sub_keys: [u32; 32], b: [u32; 4], r: [u8; 16], src: [u8; 16]) -> [u8; 16] {
    let mut b = permute_initial_block(b, src);
    for i in 0..8 {
        let s = &sub_keys[31 - 4 * i - 3..31 - 4 * i - 3 + 4];
...
fn permute_final_block(b: [u8; 16], block: [u32; 4]) -> [u8; 16] {
    let mut b = b;
    for i in 0..4 {
        b[i * 4] = (block[i] >> 24) as u8;
        b[i * 4 + 1] = (block[i] >> 16) as u8;
        b[i * 4 + 2] = (block[i] >> 8) as u8;
        b[i * 4 + 3] = block[i] as u8;
    }
    b
}
...
mod tests {
    ...
    fn test_sm4() {
        ...
        let enc = c.encrypt(original_text);
        assert_eq!(data, enc);
